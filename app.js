...
slides.forEach((slide, index) => {
  ...
  timeline
    ...
    .fromTo(img, {scale: 3}, {scale: 1}, '-=1')
    .fromTo(revealText, {x: '0%'}, {x: '-100%'}, '-=0.5')
  ...
  const pageTimline = gsap.timeline(pageOptions);
  const next = slides.length - 1 === index ? null : slides[index + 1]

  pageTimline
    .fromTo(next, {y: '0%'}, {y: '50%'})
    .fromTo(slide, {opacity: 1, scale: 1}, {opacity: 0, scale: 0.5})
    .fromTo(next, {y: '50%'}, {y: '0%'})
